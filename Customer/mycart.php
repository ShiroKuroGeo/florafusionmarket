    ...
    <div id="cart">
        ...
        <div class="mt-20">
            <div class="max-w-5xl mx-auto mt-8 p-4 bg-white rounded shadow">
                ...
                <div v-for="sh of shopname">
                    <table class="w-full border-collapse mt-5">
                        <thead>
                            <tr>
                                <th colspan="6" class="py-2 px-4 text-left border-b flex items-center mt-5">
                                    <i class="fas fa-store mr-2"></i><a href="seller_shop.php">{{ sh.shop_name }}</a>
                                </th>
                            </tr>
                            <tr class="border-b">
                        ...
                        <tbody>
                            <tr class="border-b" v-for="cart in carts">
                                <td class="py-2 px-4" v-if="cart.shop_name === sh.shop_name">
                                    <img :src="'../assets/img/' + cart.pimage" class="your-image-class w-40">
                                </td>
                                <td class="py-2 px-4" v-if="cart.shop_name === sh.shop_name">
                                    <h3 class="text-lg font-semibold mt-2">{{ cart.p_name }}</h3>
                                </td>
                                <td class="py-2 px-4" v-if="cart.shop_name === sh.shop_name">P{{ cart.p_price }}</td>
                                <td class="py-2 px-4" v-if="cart.shop_name === sh.shop_name">
                                    <div class="flex items-center">
                                        <button @click="decrement(cart)" class="text-gray-500 hover:text-blue-500 focus:outline-none">
                                            <i class="fas fa-minus"></i>
                                        </button>
                                        <span class="w-12 text-center">{{ cart.p_quantity }}</span>
                                        <button @click="increment(cart)" class="text-gray-500 hover:text-blue-500 focus:outline-none">
                                            <i class="fas fa-plus"></i>
                                        </button>
                                    </div>
                                </td>
                                <td class="py-2 px-4" v-if="cart.shop_name === sh.shop_name">{{ cart.p_totalPrice }}</td>
                                <td class="py-2 px-4" v-if="cart.shop_name === sh.shop_name">
                                    <button class="text-red-500 hover:text-red-700 focus:outline-none" @click="deleteCart(cart.id)">
                    ...
                    <button @click="checkout(sh.ids)" id="order-details-button" class="bg-blue-500 text-white py-2 px-4 ml-3 rounded hover:bg-blue-600 focus:outline-none">
                        Checkout 
                    </button>
                </div>
                        ...
                        <div class="modal-content py-4 text-left px-6 flex-1">
                            ...
                            <div class="payment-method-container flex-1 bg-gray-100 py-4 px-6">
                                ...
                                <div class="mb-4">
                                    ...
                                    <select name="paymethod" v-model="paymentMethodModel" required class="form-select border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="mode-of-payment">
                                        <option selected disabled value="0">Select Payment Method</option>
                                        <option value="1">GCash</option>
                                        <option value="2">Cash on Delivery</option>
                                    </select>
                                </div>
                                <div id="qrCodeContainer" :class="paymentMethodModel == 0 ? 'hidden' : 'mt-5'">
                                    <label class="block text-gray-700 text-sm font-bold mb-2">QR Code Image</label>
                                    <img :src="'../assets/img/' + selectedqrCoding" alt="Displayed QR Code" width="100">
                                </div>
                                <div id="proof-of-payment" :class="paymentMethodModel == 0 ? 'hidden' : 'mt-5'">
